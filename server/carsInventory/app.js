...
/*
const carsData = Promise.resolve()
    .then(() => fs.readFileSync('./data/car_records.json'))
    .then(res => JSON.parse(res))
    .catch(err => console.error('Error in loading car records:', err));
*/
...
Cars.deleteMany({})
    ...
    .catch(err => console.error('Error preparing cars data:', err));

Cars.findOne({})
    .then(res => console.log(res))
    .catch(err => console.error('Error finding a single car:', err));
...
app.route('/cars/:id', async (req, res) => {
    try {
        const documents = await Cars.find({dealer_id: req.params.id});
        res.json(documents);
    } catch (error) {
        res.status(500).json({error: 'Error fetching cars by id'});
    }
});

app.route('/carsbymake/:id/:make', async (req, res) => {
    try {
        ...
        res.status(500).json({error: 'Error fetching cars by id and make'});
    }
});
